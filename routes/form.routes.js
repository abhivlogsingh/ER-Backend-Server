...
// Define routes
router.post('/', upload.single('image'), multerErrorHandler, formController.createForm); // Create a form
router.get('/', formController.getAllForms); // Get all forms
router.get('/:id', formController.getFormById); // Get a specific form by ID
router.put('/:id', upload.single('image'), multerErrorHandler, formController.updateForm); // Update a form by ID
